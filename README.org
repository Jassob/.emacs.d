...
** TODO Version control
   ...
   #+begin_src emacs-lisp :tangle init.el
     ...
     (
       ...
       (let ((magit-auth-file (concat etc-dir "magithub-auth-file")))
	 (if (file-exists-p magit-auth-file)
	     (let-alist (read-from-file magit-auth-file)
	       (progn
		 (setq ghub-base-url .ghub-base-url)
		 (setq ghub-username .ghub-username)
		 (setq ghub-token    .ghub-token)))
	   (warn (concat "magithub: Could not find file: " magit-auth-file)))))
   #+end_src
   ...
   #+begin_src emacs-lisp :tangle init.el
     (defun magithub--set-auth-info ()
       "Prompts the user for credentials for magithub and store them in etc-dir/magithub-auth-file."
       (let ((auth-alist (make-local-variable 'auth-alist)))
	 (setq auth-alist '())
	 (dolist (key '((ghub-base-url . "Github base URL: ")
			(ghub-username . "Github username: ")
			(ghub-token . "Github token: ")))
	   (add-to-list 'auth-alist (cons (car key) (read-from-minibuffer (cdr key)))))
	 (print-to-file (concat etc-dir "magithub-auth-file") auth-alist)
	 (message "Stored magithub credentials in `%s'" (concat etc-dir "magithub-auth-file"))))
   #+end_src
...
* Reading
  ...
  when scolling with a touchpad. So I disable ~pdf-view-continous~.

  #+begin_src emacs-lisp :tangle init.el
    (use-package pdf-tools :ensure t :commands (pdf-tools-install)
      :init (add-to-list 'auto-mode-alist pdf-tools-auto-mode-alist-entry)
      :config (setq pdf-view-continuous nil))
  #+end_src
* Communication
  ...
  #+begin_src emacs-lisp :tangle init.el
    ...
    . PORT) . PASSWORD)."
      (let ((auth-file (concat etc-dir "weechat-auth-file.el")))
	(if (file-exists-p auth-file)
	    (let ((passwd-alist (read-from-file auth-file)))
