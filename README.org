...
** Golang
   When writing code in Go I want to activate the common code minor
   mode and have the Go tools do the work for me.

   #+name: package/go-mode
   #+begin_src emacs-lisp :tangle init.el
     (use-package go-mode
       :init
...
* Different configs
  ...
  #+begin_src emacs-lisp :tangle go.el :noweb yes
    <<code-config>>
    <<package/go-mode>>
  #+end_src
